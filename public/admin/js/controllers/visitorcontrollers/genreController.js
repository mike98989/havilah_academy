    ...
    /////////////////////////
  module.controller('genreController', ['$scope','$http','infogathering','all_users','misc_functions','user_session', function($scope, $http, datagrab, all_users, misc_functions, user_session) {
    ...
    $scope.genre_url= datagrab.urlSplit[2]; 
    ...
    $http.get("http://"+datagrab.dirlocation+"api/genre_content?genre_url="+datagrab.urlSplit[2])
    .then(function(response) {
    $scope.content = angular.fromJson(response.data.content);
    $scope.genre = angular.fromJson(response.data.genre);
    },function errorCallback(response) {
    ...
    ////////////PLAY SONG 
      $scope.playsong = function(author, song_title, path, cover, id){
      $scope.play = misc_functions.playsong(author, song_title, path, cover, id);    
     }
